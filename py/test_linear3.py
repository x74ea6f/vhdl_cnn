...
x_pre = np.clip(np.round(x_pre_org * 128*256), -128*256, 127*256)
w = np.clip(np.round(w_org * 128*256), -128*256, 127*256)
b = np.clip(np.round(b_org * 128*256), -128*256, 127*256)
exp = np.clip(np.round(exp_org * 128*256), -128*256, 127*256)

x_s = np.dot(w, x_pre)
## x_s = np.round(x_s / scale_x + offset_x)
x_s = np.round(x_s / 128/256)
x_s = np.clip(x_s, -128*256, 127*256)
x_s = x_s + b
x_s = np.clip(x_s, -128*256, 127*256)
print("X_S", x_s.shape)
print(x_s)
